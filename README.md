# Timoty Mercado — Portfolio (multi-file structure)
    ...
    script.js       → all interactivity (scroll reveals, ripple clicks, tilt, lightbox gallery)
    assets/
      avatar.jpg    → profile photo
      gallery/      → 23 project screenshots (used by the lightbox on each project card)

External dependency: Google Fonts (loaded via <link> tags in index.html's <head> — needs internet for the fonts to load; everything else is local).
